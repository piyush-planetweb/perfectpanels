...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Pws_Panel::cutline_edit');
    }
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Pws_Panel::panel')
            ...
            ->addBreadcrumb(__('Manage Panels'), __('Manage Cutlist'));
        return $resultPage;
    }
    ...
    public function execute()
    {
    	...
    	if($id){
    		$model->load($id);
    		if(!$model->getId()){
    			$this->messageManager->addError(__('This Cutlist no longer exits. '));
        ...
        $resultPage->addBreadcrumb(
            $id ? __('Edit Panel') : __('New Cutlist'),
            $id ? __('Edit Panel') : __('New Cutlist')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Cutlist'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getname() : __('New Cutlist'));
