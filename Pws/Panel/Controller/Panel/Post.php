        ...
        ->addFieldToFilter('url_key', $cusid);
            ...
            try {
				...
				if(isset($data['mail_send'])){
					...
					$transport = $this->_objectManager->create('Excellence\EmailAttachment\Mail\Template\TransportBuilder')->setTemplateIdentifier(4)
									->setTemplateOptions($templateOptions)
									->setTemplateVars($templateVars)
									->setFrom($from)
									->addTo($adminemail)
									->addTo('alkabagga@gmail.com')
									//->addTo('stivemarks2@gmail.com')
									->getTransport();
					$transport->sendMessage();
					
					$touser = array($useremail);
					$transportnew = $this->_objectManager->create('Excellence\EmailAttachment\Mail\Template\TransportBuilder')->setTemplateIdentifier(3)
									->setTemplateOptions($templateOptions)
									->setTemplateVars($templateVars)
									->setFrom($from)
									->addTo($useremail)
